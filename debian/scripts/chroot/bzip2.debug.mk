...
EPOCHNS=$$(date +%s%N); \
echo $$EPOCHNS > $(LOG_DIR)/$$KEY.start; \
echo "START:  $$TS --- $@"
endef
...
TS=$$(date +'%Y-%m-%d %H:%M:%S.%N'); \
EPOCHNS=$$(date +%s%N); \
ST=$$(cat $(LOG_DIR)/$$KEY.start 2>/dev/null || echo 0); \
ELAPSED=$$(awk -v s="$$ST" -v e="$$EPOCHNS" 'BEGIN{d=e-s; printf "%.3f", d/1e9}'); \
echo "FINISH: $$TS --- $@ (elapsed $$ELAPSED s)"
endef
