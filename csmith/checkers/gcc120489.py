...
def is_in_comment(line: str, in_multiline_comment: bool) -> tuple[bool, bool]:
    ...
    if '//' in line:
        ...
        # Check if there's "continue" before the comment
        before_comment = line[:comment_pos].strip()
        if before_comment and 'continue' in before_comment:
            return False, False
        return True, False
    ...
    if '/*' in line:
        start_pos = line.find('/*')
        # Check if there's "continue" before the comment starts
...
def analyze_file(filepath: str) -> List[Dict[str, Any]]:
    """
    Analyzes a single file for patterns matching GCC bug 120489.
    """
    try:
    ...
    for i, line in enumerate(lines):
        ...
        # Only check for "continue" if not in a comment
        if not is_comment and 'continue' in line:
            found_patterns.append({
                "file": filepath,
                "line": i + 1,
                "code": line.strip(),
                "reason": "The keyword 'continue' was found on this line."
            })
...
def main():
    ...
    if not all_findings:
        print("No potential bug patterns found for GCC bug 120489.")
        sys.exit(0)

    print(f"Found {len(all_findings)} instances of the 'continue' keyword.")
    print("This checker looks for lines containing 'continue' while excluding comment sections.")
    print("-" * 60)
