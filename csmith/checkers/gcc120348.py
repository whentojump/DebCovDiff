...
def analyze_file(filepath: str) -> List[Dict[str, Any]]:
    """Analyzes a single file for `extern "C"` patterns."""
    try:
    ...
    found_patterns = []
    # Regex to find `extern "C"`, allowing for any amount of whitespace between them.
    extern_c_regex = re.compile(r'\bextern\s*"C"')

    for i, line in enumerate(lines):
        if extern_c_regex.search(line):
            found_patterns.append({
                "file": filepath,
                "line": i + 1,
                "code": line.strip(),
                "reason": "An 'extern \"C\"' linkage specification was found."
            })
...
def main():
    ...
    if not all_findings:
        print("No 'extern \"C\"' patterns found.")
        sys.exit(0)

    print(f"Found {len(all_findings)} instances of the 'extern \"C\"' pattern.")
    print("-" * 60)
    ...
    print("\nDisclaimer:")
    print("This script provides an approximation based on syntactic patterns.")
